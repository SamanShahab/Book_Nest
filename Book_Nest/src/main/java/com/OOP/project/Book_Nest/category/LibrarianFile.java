...
//    private static final int FINE_PER_DAY = 10; // Fine per day in currency units
//
//    public List<String> getBooksByCategory(String category) {
//        List<String> books = new ArrayList<>();
//        try (BufferedReader reader = new BufferedReader(new FileReader("C:\\Users\\hp hp\\Downloads\\Book_Nest\\src\\main\\java\\com\\OOP\\project\\Book_Nest\\booksData.txt"))) {
...
//                String[] parts = line.split(":");
//                if (parts.length >= 2 && parts[0].equals(category)) {
//                    books.add(parts[1].split("\\(")[0].trim());
//                }
//            }
//        } catch (IOException e) {
//            e.printStackTrace();
...
//                    return false;
//                } else if (line.contains(book)) {
//                    updatedLines.add(line + " (Borrowed:" + LocalDate.now().format(DATE_FORMATTER) + ")");
//                } else {
//                    updatedLines.add(line);
//                }
//            }
...
//            for (String line : lines) {
//                if (line.contains(book) && line.contains("(Borrowed:")) {
//                    String borrowedDateStr = line.substring(line.indexOf("(Borrowed:") + 10, line.indexOf(")"));
//                    LocalDate borrowedDate = LocalDate.parse(borrowedDateStr, DATE_FORMATTER);
//                    LocalDate today = LocalDate.now();
//
//                    long daysOverdue = java.time.temporal.ChronoUnit.DAYS.between(borrowedDate, today) - BORROW_DURATION_DAYS;
//                    if (daysOverdue > 0) {
//                        int fine = (int) (daysOverdue * FINE_PER_DAY);
//                        JOptionPane.showMessageDialog(null, "Book is overdue by " + daysOverdue + " days. Fine: " + fine + " units.");
//                    } else {
//                        JOptionPane.showMessageDialog(null, "Book returned on time. No fine.");
//                    }
//
//                    updatedLines.add(line.replaceFirst(" \\(Borrowed:.*\\)", ""));
//                } else {
//                    updatedLines.add(line);
//                }
//            }
